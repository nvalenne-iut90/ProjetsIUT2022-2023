import {Perso} from "@/services/data.service";
...
async function getAllCharacs() {
    ...
    try {
        ...
        // NB : pour la liste des persos, il ne devrait jamais y avoir d'erreur. Au pire,
        // on obtient un tableau vide.
        if (response.error === 0) {
            // les données de la réponse sont un tableau contenant les persos
            let characs = []
        ...
        response = {error: 1, data: 'erreur réseau, impossible de récupérer la liste des persos'  }
    }
    return response
}
